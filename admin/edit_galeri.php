...
$id = $_GET['id'];
$b = mysqli_fetch_array(mysqli_query($conect, "SELECT * FROM tb_galeri WHERE id_galeri='$id'"));
...
if (isset($_POST["submit"])) {
    ...
    if(empty($namagambar)){
        $sql = mysqli_query($conect, "UPDATE `tb_galeri` SET `id_member`='$id_member' WHERE id_galeri='$id'");
        if ($sql) {
        ...
        $sql = mysqli_query($conect, "UPDATE `tb_galeri` SET `id_member`='$id_member',`gambar`='$newnama' WHERE id_galeri ='$id'");
        if ($sql) {
            echo "<script>alert('input berhasil');document.location='galeri.php'</script>";
        } else {
            echo "<script>alert('input gagal silahkan coba lagi');</script>";
